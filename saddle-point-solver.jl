...
const xGauss = gausshermite(71)[1]
const wGauss = gausshermite(71)[2]
...
function RSB1_SP_fixM(beta, m_target;
    ...
    for i in 1:maxsteps
        ...
        new = [new_h, new_q0, new_q1, new_p1]
        current = [current_h, current_q0, current_q1, current_p1]
        xerr = max(abs.(new .- current) ./ current ...)
        ...
        if xerr < tol
            return (beta, current_h, m_target, current_q0, current_q1, current_p1, xerr, 1., i)
        end
    end

    return (beta, current_h, m_target, current_q0, current_q1, current_p1, xerr, 0., maxsteps)
end
